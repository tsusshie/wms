...
if ( $val == NULL){
    ...
    <table>
        <tr>
            <td><input type="button" value="OK" onclick="location.href='configure.php?action=yes'" /></td>
...
function resettypetable()
{
    ...
    if ( $dbi->query($query)){
        $query = "INSERT INTO type_master (id, type) VALUES (0, '無効'),(1, '輸出者'),(2, '顧客')";
        if ($dbi->query($query)){
            $ret = TRUE;
        }
    }
